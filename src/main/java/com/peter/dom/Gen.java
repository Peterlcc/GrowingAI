...
public void genLaunch(File f,String lib_path) throws FileNotFoundException
{
	PrintStream psOld = System.out;
	System.setOut(new PrintStream(f));
	String include_context=readFile(lib_path+"\\fake_move_base_amcl.launch");
	System.out.println(include_context);
	System.setOut(psOld);
}
...
public void gen_code_different_version() throws IOException {
		...
		if(package_name != null && !package_name.equals("")) {
			...
			File launch = new File(root_path+ "\\fake_move_base_amcl.launch");
			launch.createNewFile();
			genLaunch(launch,lib_path);
